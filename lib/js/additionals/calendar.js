...
$(function() 
{
    ...
    ({
        ...
        editable: true,
        eventSources: [{url:'includes/cal_events.php', allDayDefault: false}],
        eventDrop: 
			function(event) 
			{ 
				$.post('includes/cal_update.php', event, function(response){
...
function save()
{
	$("form#add_event").on('submit', function(e){
		$.post('includes/cal_save.php', $(this).serialize(), function(response) {
			if(response == 1) {
...
function update(id, title, description)
{
	...
	$.post('includes/cal_edit_update.php', construct, function(response) {
		if(response == '') {
