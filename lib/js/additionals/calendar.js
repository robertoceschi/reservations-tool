...
$(function() 
{
    ...
    ({
        ...
        editable: true,
        eventSources: [{url:APPPATH +'includes/cal_events.php', allDayDefault: false}],
        eventDrop: 
			function(event) 
			{ 
				$.post(APPPATH + 'includes/cal_update.php', event, function(response){
...
function save()
{
	$("form#add_event").on('submit', function(e){
		$.post(APPPATH +'includes/cal_save.php', $(this).serialize(), function(response) {
			if(response == 1) {
...
function remove(id)
{
	...
	$.post(APPPATH +'includes/cal_delete.php', construct, function(response) {
		if(response == '') {
			$("#cal_viewModal").modal('hide');
...
function update(id, title, description)
{
	...
	$.post(APPPATH +'includes/cal_edit_update.php', construct, function(response) {
		if(response == '') {
