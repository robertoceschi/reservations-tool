...
$(function() 
{
    ...
    ({
        ...
        editable: true,
        eventSources: [{url:'http://localhost:8888/projekte/reservations-tool/application/includes/cal_events.php', allDayDefault: false}],
        eventDrop: 
			function(event) 
			{ 
				$.post('http://localhost:8888/projekte/reservations-tool/application/includes/cal_update.php', event, function(response){
...
function save()
{
	$("form#add_event").on('submit', function(e){
		$.post('http://localhost:8888/projekte/reservations-tool/application/includes/cal_save.php', $(this).serialize(), function(response) {
			if(response == 1) {
...
function remove(id)
{
	...
	$.post('includes/cal_delete.php', construct, function(response) {
		if(response == '') {
			$("#cal_viewModal").modal('hide');
...
function update(id, title, description)
{
	...
	$.post('http://localhost:8888/projekte/reservations-tool/application/includes/cal_edit_update.php', construct, function(response) {
		if(response == '') {
