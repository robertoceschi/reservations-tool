        ...
        ({
            ...
            eventRender:function (event, element) {
                ...
                if (event.url == undefined) {
                    ...
                    if (event.user_id == 0) {
                        element.tooltip();
                        element.attr('onclick', 'javascript:openModal_new_res("' + event.title + '","' + event.start + '","' + event.end + '" , "' + event.description + '","' + event.id + '","' + event.user_id + '" );');
                    }
...
function openModal(title, start, end, info, id, user_id) {
    ...
    if (user_id == 0 || active_user_id == user_id) {

        $(".modal-body").html('von: ' + start + '<br/>' + 'bis: ' + end);
        $(".modal-header h4").html('Court reservieren');
...
function openModal_new_res(title, start, end, info, id, user_id) {
    var title = '';

    $(".modal-header h4").html('Court reservieren');
    $(".modal-body").html('Wollen Sie sie diesen Termin von ' + start + 'bis ' + end + 'reservieren?');
    $("#cal_editModal").modal('show');
    // Delete button => delete Event
    ...
    $(".modal-footer").delegate('[data-option="save"]', 'click', function (e) {
        ...
        var event_title = $("form#event_title").serialize();
        var event_description = $("form#event_description").serialize();
        update(id, event_title, event_description, user_id);
        e.preventDefault();
    });
}
